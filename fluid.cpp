...
class FluidCell {
    public:
        ...
        float getTemp(bool recalculate=false) {
            if (recalculate) {
                float cv = consts::r/consts::HMol/(1.4-1);
...
void FluidGrid::update(SDL_Event event) {
    ...
    if (compressible) compressibleMomentumUpdate();
    else projection(5);
