...
class FluidCell {
    ...
    private:
        float mass, size, density, temperature, pressure, width, height, smokeMass, e, E, gravPotential;
        bool compressible;
        int row, col;
...
FluidGrid::FluidGrid(float width, float height, int r, int c, float dt, bool comp): width(width), height(height), rows(r), cols(c), dt(dt), compressible(comp) {
    ...
    for (i = 0; i < rows; i++) {
        ...
        for (j = 0; j < cols; j++) {
            //random mass for now
            // float mass = (std::rand() % 10000)+10;
            std::vector<uint> color = {0, 0, 0};
            grid[i][j] = FluidCell(1e23, cellWidth, cellHeight, 400, 0,0,0,comp);
            if (i == rows/2 && j == cols/2) {
                grid[i][j] = FluidCell(1e25, cellWidth, cellHeight, 400, 0,0,0,comp);
            }
            // } else if (i == rows-1) {
...
void FluidGrid::energyUpdate() {
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            ...
            float lambda = combinedCoolingFunction(density,T);
            if (E + lambda * getdt() >= 0) {
                E += -lambda*getdt();
            } else {
                E = 0.0f;
...
void FluidGrid::setVelocities(bool setE=false, bool noRecalc=false) {
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            ...
            if (cell->getTemp(false)) {
                maxT = cell->getTemp(false);
            }
        ...
        if (buttonEvent.button == SDL_BUTTON_RIGHT) {
            ...
            switch (event.key.keysym.sym) {
                ...
                case SDLK_d:
                    ...
                    printf("dt: %f\n", 0.1 * std::min(cellWidth, cellHeight)/maxV);
                    break;
                case SDLK_t:
                    densityDisplay = 0;
                    pressureDisplay = 0;
